...
$buku = mysqli_fetch_assoc(mysqli_query($connect, "SELECT SUM(stok) AS jb FROM buku"));
$jumlah_buku = $buku["jb"];

$anggota = mysqli_fetch_assoc(mysqli_query($connect, "SELECT COUNT(*) AS ja FROM anggota"));
$jumlah_anggota = $anggota["ja"];

$pinjam = mysqli_fetch_assoc(mysqli_query($connect, "SELECT COUNT(*) AS jt FROM peminjaman"));
